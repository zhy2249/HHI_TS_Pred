...
2026-09-26 全量接入更新：**剩余16组也已实现**，当前 MODE 1..24 均可运行，见 [全量实现说明](TS_Predictor_R8_All24_Implementation.md)。下文保留首八组交付时的算法边界、默认批量清单与验收记录；新23/24的量化搜索改动不适用于旧八组。

2026-09-26结果更新：已收到七组完整LB CE，19尚未完成，见 [结果分析](TS_Predictor_R8_First7_Results_20260926.md)。
下文的实现/运行步骤保持；文中“本轮未运行”的描述指2026-09-25本地实现验收，不否认后续服务器结果。

算法规格：`R8-DESIGN-20260925-v2`。下文为首八组接入记录；其余编号现已补齐，不再是预留模式。
...
宏选择算法，不用CMake选算法。宏互斥和master-OFF检查均在编译时执行；当前有效值0..24，非法编号报错。
