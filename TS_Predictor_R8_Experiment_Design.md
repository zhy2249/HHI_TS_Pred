...
首轮已实现1/4/8/13/15/16/17/19，其余非零值仍为预留，误请求报错而不是静默使用Current。
...
这些是原设计阶段的独立 Python 数学规格验证，本身不是 codec smoke 或实际收益；后续已完成C++逐项对照，分开记录在验收文档。
...
共用现有 batch 共享任务池，某组最后几个任务未完成时下一组仍可占空槽；每组完成立即复制模板、写该组 XLSM，重试后刷新；Reference 不改，无重建输出。当前24组runtime已注册，运行命令见全量实现说明；原首八组包装脚本的默认清单保持不变。
