# R8-20 / B08：r8_20_smoothed_all_support

状态：2026-09-26 已实现并接入宏/运行时选择；尚无本组正式 CTC 结果。见 [全量实现与验证](../../../TS_Predictor_R8_All24_Implementation.md)。

- 启用宏：`JVET_BJUT_TS_R8_MODE=20`；须重新编译 Encoder/Decoder。
- 运行时覆盖：`r8_smoothed_all_support`；现有 batch 已注册，使用同一共享任务池。
- cost / samples：`fractional10` / `smooth_121`。
- candidates / decision / sparse：`Psmooth` / `raw` / `smooth_if_n_positive`。
- quantization search：`native`；实施批次：2。
- 直接机制对照：B07；正式 anchor 始终为 Current。
