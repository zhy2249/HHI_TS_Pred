...
- 预留宏：`JVET_BJUT_TS_R8_MODE=17`；当前编码器尚不支持。
- 预留 runtime：`r8_minimax`；当前 batch 尚不接受此名称。
- cost / samples：`fractional10` / `empirical`。
- candidates / decision / sparse：`P0` / `minimax_regret` / `Smax`。
- quantization search：`native`；实施批次：1a。
- 直接机制对照：A01、A04；正式 anchor 始终为 Current。
